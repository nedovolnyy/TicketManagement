...
namespace TicketManagement.IntegrationTests.Database
{
    public class EventSeatRepositoryTests
    {
        ...
        {
            // arrange
            var expectedEventSeat = new EventSeat(0, 2, 9, 1, State.NotAvailable);
        ...
        {
            // arrange
            var expectedEventSeat = new EventSeat(1, 1, 3, 3, State.NotAvailable);

            // act
            await _eventSeatRepository.UpdateAsync(expectedEventSeat);
            var actualEventSeat = await _eventSeatRepository.GetByIdAsync(expectedEventSeat.Id);
